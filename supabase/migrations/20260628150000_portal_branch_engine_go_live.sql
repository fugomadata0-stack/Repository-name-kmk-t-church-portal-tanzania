-- Go-live: Injini ya Matawi workspace (idempotent — salama kurudia ikiwa 20260628140000 ilishindwa).
...
on public.portal_branch_engine_workspace for select to authenticated
...
on public.portal_branch_engine_workspace for insert to authenticated
...
on public.portal_branch_engine_workspace for update to authenticated
...
on public.portal_branch_engine_workspace for delete to authenticated
using (auth_user_id = auth.uid());
...
begin
  if exists (select 1 from pg_publication where pubname = 'supabase_realtime') then
    if not exists (
      ...
      where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'portal_branch_engine_workspace'
    ) then
