-- Rekebisha RPC ya rate limit kwa PostgREST / Supabase (epuka 400 kutoka digest/convert_to)
...
begin
  ...
  v_hash := encode(extensions.digest(v_scope || '|' || v_id, 'sha256'), 'hex');
  ...
  if v_row.blocked_until is not null and v_row.blocked_until > v_now then
    return query
      select false,
        greatest(1, ceil(extract(epoch from (v_row.blocked_until - v_now)))::int),
        v_row.attempts;
    return;
  end if;
