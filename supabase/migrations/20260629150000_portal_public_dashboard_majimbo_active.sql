-- Mfumo Hai strip: ongeza majimbo_active (jimbo zenye status = active) kwenye portal_public_dashboard_counts.
-- Inafuata 20260627150200_portal_public_dashboard_attendance_counts.sql
...
as $$
  ...
  select
    ...
    (
      ...
      where coalesce(lower(trim(j.status)), 'active') = 'active'
    ) as majimbo_active,
...
comment on function public.portal_public_dashboard_counts() is
  'Aggregate public landing totals; includes majimbo_active, matawi breakdown, and attendance (TZ Africa/Dar_es_Salaam).';
