...
export function DashboardHero({
      ...
      <div>
        ...
        {liveAt ? (
          <p className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-emerald-400/35 bg-emerald-500/15 px-3 py-1 text-[11px] font-semibold text-emerald-100">
            <Activity className={`h-3.5 w-3.5 ${reducedMotion ? "" : "animate-pulse"}`} aria-hidden />
  ...
  const shellClass =
    "relative mb-6 overflow-hidden rounded-2xl border border-[#D4AF37]/30 bg-gradient-to-br from-[#061633] via-[#123C69] to-[#061633] p-5 shadow-2xl sm:p-7";

  const decor = reducedMotion ? (
    <>
      <span className="pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-amber-400/15 blur-3xl" aria-hidden />
      <span className="pointer-events-none absolute -left-8 bottom-0 h-32 w-32 rounded-full bg-sky-400/10 blur-2xl" aria-hidden />
    </>
  ) : (
    <>
